...
## Structure
Six files in this folder (all open directly in a browser). The in-product files (`index`, `dynatrace`, `datadog`, and the `dynatrace-v2` variant) all reuse the **same real APM chrome** — each is a copy of `index.html`'s live-8.2.6 Side_bar_menu shell, boots into `APM ▸ Profiling`, keeps the real APM tab bar (Services · Explorer · Error Tracker · Compare · **Profiling NEW**), and swaps only the `#pfBody` profiling flow (all `pf-`prefixed styles + an isolated IIFE exposing `window.initProfiling`, so nothing collides with the shell). Non-profiling APM tabs show a "mirrors live current state" stub — **except in `dynatrace-v2`, where the Services tab is fully built out** (see its bullet below).
...
- **`apm-profiling-dynatrace.html`** — **in-product Dynatrace-modelled** flow (process-centric): overview (CPU chart + process grid) → details (split-by chart + child processes + 4-signal tabs Response time/Failure rate/CPU/Throughput) → method hotspots & analyze outliers → top contributors → create-analysis-view. Has a **doc-grounded data-availability badge system** (green "Live in APM" = Latency/Error Rate/Throughput/traces; orange "Requires profiler" = CPU consumption/GC/method hotspots/per-request CPU; the CPU signal tab shows a *locked* state, not a fake chart). The APM header shows a **dynamic breadcrumb** (`APM ▸ Profiling ▸ <service> ▸ <screen>` via `setHead()`) that updates on every navigation.
- **`apm-profiling-dynatrace-v2.html`** — the **richest** deliverable: a variant of the Dynatrace flow (cloned from `apm-profiling-dynatrace.html`) with the **Top contributors screen removed**, a **device-monitor drill-down** added, and — unique to this file — a **real APM ▸ Services tab** and a **per-service page** that hosts the profiling flow as a new tab. It therefore has **two entry points into the same profiling screens**:
  - **Top-level `APM ▸ Profiling`** — overview → process details → click a **process** row → **Attached services** list (`pfv-services`) → click a **service** → **device-monitor template** (`pfv-monitor`, modelled on the live Motadata Windows-agent dashboard: host header + severity 1/9/7, 6 stat cards, red "Up 0%" availability donut, availability-stats bars, Service Status + Process Details tables — in the host's *Down / No data* empty state; IP scrubbed to `192.0.2.208`).
  - **`APM ▸ Services`** (`pfv-apmsvc`) — mirrors the **live 8.2.6 service card grid**: 9 services with severity ring, tech logo, Response Time / Throughput area sparklines and an Error Count bar sparkline, plus a working search filter and a card ↔ list view toggle. All `pfs-`prefixed.
  - **`APM ▸ Services ▸ <service>`** (`pfv-svcdetail`) — the **live service page**, which *replaces* the APM head + tab bar exactly like the real product (`svdChrome()` hides `.pagehead` + `#pfApmTabs`). Header = back chevron · globe · `Integration | <service>` · time picker; tabs = Overview · Transactions · API Endpoint · Database · JVM · Error Tracker · Logs · **Profiling NEW**. Overview has the 6 KPI cards, the 3 axis charts (Response Time w/ `service.trace.duration.*` legend, Request Throughput, stacked Requests & Errors) and a **Service Map** (severity-ringed globe nodes + arrowed edges + zoom cluster). The six live tabs show a short "mirrors live current state" panel. All `svd-`prefixed.
  - The **Profiling tab hosts the real flow**, starting at **Process details**. There is only **one copy** of the four flow views (`pfv-detail`/`pfv-services`/`pfv-monitor`/`pfv-method`): they live in `#pfBody` for the top-level tab and are **moved into `#svdProf`** while the service page's Profiling tab is open (`svdProfHome()` / `svdProfMount()`). A **`pfGo()` dispatcher** routes every `data-goto` and row click to `goSvcProf()` (in-tab) or `go()` (top-level), so the whole drill-down works from both places.
  - Because the flow is reachable per service, the **process grid and attached-services list are keyed to the selected service** via `PROCSPEC` (9 services × 2 processes + 4 attached services, deterministic seeded numbers) — `renderProcs()` / `renderAttached()`.
- **`apm-profiling-datadog.html`** — **in-product Datadog-modelled** flow (flame-graph-first): overview = "Select service to view flame graph" (expandable service list + View Flame Graph) → details with View tabs (Flame Graph · Thread Timeline · Metrics · Profile List · Call Graph) + metric selector (CPU Time · Wall Time · Allocated Memory · Live Heap · Lock · Socket I/O · Exceptions) + endpoint/function side panel. The flame detail carries a per-service **"Processes" panel** (the service's profiled runtimes keyed from `SVCS[].procs` — host/container/process_id/version; aggregate-across-all with **scope-to-one-runtime**, + a "Show runtimes" table). Its flame graph is **interactive & collapsible**: nodes carry stable `_id`s, a `COLLAPSED` set folds subtrees, it **defaults to fully collapsed** (root + one level), **click a frame to expand one level** / caret ▾▸ to toggle / click an expanded frame to zoom, plus **Expand-all / Collapse-all icon buttons** and Reset-zoom. (This flame machinery started as `index.html`'s but now **diverges** — it added the collapse state; index/dynatrace still render fully-expanded.)
...
All six share a fixed bottom-center **deliverable switcher** (`#deliv-switch`): five numbered options **1 In‑product** · **2 Prototype** · **3 Dynatrace** · **4 Datadog** · **5 Dynatrace v2**, plus a separate **📄 Notes** link (the redesign doc). Each file highlights its own entry — to add/reorder, edit the switcher block in **every** file (it's duplicated per file).

The Dynatrace & Datadog in-product flows are grounded in the **live services** (javaDistributed, nodeDistributedApp, PHPApp, NodeApp, DotnetApp, pythonDistributedApp); each carries a top-of-page **feasibility banner** stating what fits the current product vs. what needs a new profiler backend. `dynatrace-v2`'s Services grid uses the **fuller 9-service list** later sampled from the live instance (the 6 above + `java-app@k8s`, `python-app@k8s`, `JavaApp`), with each service's real Response Time / Throughput / Error Count.
