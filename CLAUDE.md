...
- **`apm-profiling-dynatrace-v2.html`** — a **variant of the Dynatrace flow** (cloned from `apm-profiling-dynatrace.html`) with the **Top contributors screen removed**, and a **device-monitor drill-down** added. Flow: overview → process details → click a **process** row → **Attached services** list (`pfv-services`, 4 services w/ tech·req/min·median·host) → click a **service** → **device-monitor template** (`pfv-monitor`, modelled on the live Motadata Windows-agent dashboard: host header + severity 1/9/7, 6 stat cards, red "Up 0%" availability donut, availability-stats bars, Service Status + Process Details tables — rendered in the host's *Down / No data* empty state). The "View method hotspots / Analyze outliers" buttons still reach method hotspots. Wired via `go()` + `SCRNAME` entries `services`/`monitor`; internal host IP scrubbed to `192.0.2.208`.
